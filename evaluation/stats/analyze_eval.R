...
# Tự động kiểm tra và cài đặt các thư viện R cần thiết
...
# 1. Nạp dữ liệu đánh giá LungCare AI
json_file <- "data/evaluation_results.json"
if (!file.exists(json_file)) {
  stop("❌ Không tìm thấy file data/evaluation_results.json! Hãy chạy run_pipeline.py trước.")
}
...
# 3. Vẽ biểu đồ 1: Tỷ lệ đạt (%) của các tiêu chí nhị phân (Binary Metrics)
binary_df <- scores_df %>%
  select(case_id, guideline_adherence, safety_of_recommendations, recognition_of_key_risks, accuracy_of_grading, conversational_explanation) %>%
  ...
  mutate(Metric = case_when(
    ...
    Metric == "accuracy_of_grading" ~ "Grading Accuracy",
...
p1 <- ggplot(pass_rates, aes(x = reorder(Metric, PassRate), y = PassRate, fill = Metric)) +
  ...
  labs(
    title = "LungCare AI Clinical Evaluation (Pass Rate %)",
    subtitle = paste("Based on", nrow(scores_df), "clinical scenarios"),
...
if (!dir.exists("static")) {
  dir.create("static", recursive = TRUE)
}
...
likert_df <- scores_df %>%
  select(case_id, clarity, overall_helpfulness) %>%
  pivot_longer(cols = -case_id, names_to = "Metric", values_to = "Score") %>%
  mutate(Metric = case_when(
...
p2 <- ggplot(likert_df, aes(x = Metric, y = Score, fill = Metric)) +
  ...
  labs(
    title = "LungCare AI Quality Ratings (Likert Scale 1-5)",
    subtitle = "Rating distribution across clinical dimensions",
    x = "Quality Dimension",
    y = "Score"
  ) +
  theme_minimal(base_family = "sans") +
...
for (col in c("guideline_adherence", "safety_of_recommendations", "recognition_of_key_risks", "accuracy_of_grading", "conversational_explanation", "clarity", "overall_helpfulness")) {
  mean_val <- mean(scores_df[[col]], na.rm = TRUE)
  sd_val <- sd(scores_df[[col]], na.rm = TRUE)
  ...
  metric_name <- case_when(
    ...
    col == "accuracy_of_grading" ~ "Độ chính xác phân loại (Grading Accuracy)",
  ...
  if (col %in% c("guideline_adherence", "safety_of_recommendations", "recognition_of_key_risks", "accuracy_of_grading", "conversational_explanation")) {
    summary_text <- paste0(summary_text, sprintf("| **%s** | %.2f | %.2f | Nhị phân (0/1) | **%.1f%%** |\n", metric_name, mean_val, sd_val, mean_val * 100))
  } else {
    summary_text <- paste0(summary_text, sprintf("| **%s** | %.2f | %.2f | Thang đo | *N/A (Continuous)* |\n", metric_name, mean_val, sd_val))
  }
}

write(summary_text, file = "data/evaluation_stats.md")
cat("📝 Đã xuất báo cáo thống kê dạng bảng: data/evaluation_stats.md\n")
cat("🎉 Hoàn tất phân tích dữ liệu ung thư phổi trên R thành công!\n")
